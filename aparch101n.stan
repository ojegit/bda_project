// AGARCH(1,0,1) model for time series of returns
data {
  ...
  vector<lower=0>[4] s0_gp;   // Standard deviation for normal prior omega, alpha, and beta
...
transformed parameters {
  ...
  // Variance
  st = omega + alpha * pow(abs(u0), delta) + beta * pow(h0, delta/2); //s0 = vol0^d, vol0 = var0^0.5 or vol^2 = var0, var_t = s_t^{2/d}
  h[1] = pow(st, 2/delta);
  for (t in 2:N) {
    {
    real ut = y[t-1] - mu;
    st = omega + alpha * pow(abs(ut), delta) + beta * st;
    h[t] = pow(st, 2/delta);
    }
  }
...
generated quantities {
  ...
  {
  real ut = y[N] - mu;
  sft = omega + alpha * pow(abs(ut), delta) + beta * st;
  ...
  for (t in 2:n_steps_ahead) {
    ...
    sft = omega + alpha * pow(abs(ut), delta) + beta * sft;
